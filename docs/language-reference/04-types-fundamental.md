...
required. This includes data shared between different targets even on the same device.
...
|-----------------------|-----------------------------------------|--------------------------------------------|
| `half`, `float16_t`   | 16-bit floating-point number (IEEE 754) | 1/5/10                                     |
| `float`, `float32_t`  | 32-bit floating-point number (IEEE 754) | 1/8/23                                     |
| `double`, `float64_t` | 64-bit floating-point number (IEEE 754) | 1/11/52                                    |

Rules for rounding, denormals, infinite values, and not-a-number (NaN) values are generally
target-defined. IEEE 754 compliant targets adhere to the [IEEE
754-2019](https://doi.org/10.1109/IEEESTD.2019.8766229) standard.

All targets support the `float`/`float32_t` type. Support for other types is target-defined. See [targets
platforms](../target-compatibility.md) for details.
