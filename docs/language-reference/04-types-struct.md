...
> other layout algorithms. Any layout for structure types must guarantee an alignment at least as large as the
> standard layout.

## C-Style Layout

C-style layout for structure types differs from base layout by adding rounding up the structure size to the
structure alignment. This mirrors the layout rules used by typical C/C++ compilers.
