...
A variable may be defined with an interface type in which case it is called an interface object. An interface
object is a *variant*, which may have any type conforming to the interface. When an interface object is
...
> Remark 1: Function parameters with interface types do not impose the above restrictions.
...
# Memory layout and dynamic dispatch mechanism

The memory layout of an interface object is unspecified. Type-based dynamic dispatching of a member function
invocation is unspecified. Both are subject to change.

## Non-normative description

> Important note: The contents of this section is informational only.
